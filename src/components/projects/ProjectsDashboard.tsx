    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '36px 28px', maxWidth: '1240px', width: '100%', margin: '0 auto' }} className="ds-page-enter">
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '14px' }}>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-faint)' }}>{projects.length} workspace{projects.length !== 1 ? 's' : ''}</div>
        </div>
        ...
        {hasProjects ? (
          <div className="ds-table" style={{ marginTop: '20px' }}>
            ...
            {projects.map(p => (
              <div key={p.id} className="ds-table-row" onClick={() => router.push(`/projects/${p.id}`)}>
                <div style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: '13.5px' }}>{p.name}</div>
                <div>
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: '12px' }}>{formatRelativeTime(p.updatedAt)}</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{p.status === 'active' ? 'Active' : 'Paused'}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            <div style={{ marginTop: '26px', fontSize: '20px', fontWeight: 700, color: 'var(--text-primary)' }}>
              No active workspaces detected.
            </div>
            <div style={{ marginTop: '12px', maxWidth: '520px', color: 'var(--text-muted)', fontSize: '13px', lineHeight: 1.65 }}>
